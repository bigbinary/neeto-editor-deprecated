...
export default class RichTextEditor extends Component {
  ...
  componentDidMount() {
    const { handleChange, handleFocus, focusEditor, editorHeight, fixToolbar } = this.props;
  ...
  componentWillUnmount() {
    ...
    if (this.props.fixToolbar) window.removeEventListener("scroll", this.handleScroll);
  }
  ...
  handleScroll = () => {
    if (this.props.fixToolbar && this.trix) {
      ...
      if (!this.state.isSticky && top < fixOffsetTop && bottom > toolBarHeight) {
        this.setState({ isSticky: true });
      } else if (this.state.isSticky && (top > fixOffsetTop || bottom < toolBarHeight)) {
  ...
  render() {
    ...
    return (
      <div className={classNames("flex flex-col items-start justify-start flex-grow", [className])}>
        {label && <label className="mb-1 ne-label">{label}</label>}
        <div className="ne-trix__wrapper" ref={(element) => (this.trix = element)}>
          <EditorToolbar
            id={`editorToolbar_${id}`}
            toolbarStyles={toolbarStyles}
            insertImage={showImageUpload && this.insertImage}
            attachmentRef={attachmentRef}
          />
          <input id={`editor_${id}`} value={value} type="hidden" name="content" />
          <trix-editor
            name={name}
            id={`trixEditor_${id}`}
            input={`editor_${id}`}
            ...
            data-direct-upload-url={uploadUrl || DIRECT_UPLOAD_API_URL}
            data-blob-url-template={blobUrl || DIRECT_UPLOAD_BLOB_TEMPLATE_URL}
