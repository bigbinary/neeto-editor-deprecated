...
export default class FormRichText extends Component {
  state = {
    editorHeight: 400,
    isSticky: false,
  };
  ...
  renderFormInputError = () => {
    ...
    return <span className="mt-1 text-xs text-red-600">{message}</span>;
  };
      ...
      <React.Fragment>
        {label && (
          <Label className="field-label mb-1">
            {label}
            {this.props.required && (
              <i className="ri-asterisk ri-xs align-text-top ml-0.5"></i>
            )}
          </Label>
        )}
        <div className="trix col-12" ref={(element) => (this.trix = element)}>
          <EditorToolbar
            id={`editorToolbar_${id}`}
            toolbarStyles={toolbarStyles}
            insertImage={this.insertImage}
            attachmentRef={this.props.attachmentRef}
            isCompressedToolbar={this.props.isCompressedToolbar}
          />
          <input
            id={`editor_${id}`}
            value={value}
